...
public class BaseVideoActivity extends Activity {
        ...
        private void check(String result) {
            try {
                ...
                String json = face.getJSONObject(0).toString();
                Gson gson = new Gson();
                FaceBean faceBean = gson.fromJson(json, FaceBean.class);
                faceBean.isOpenMouth();
                faceBean.isShake();
                //Log.w("test", "摇头 " + faceBean.isShake());
                //Log.w("test", "张嘴 " + faceBean.isOpenMouth());
                //Log.w("test", "解析完毕 " + (System.currentTimeMillis() - time1));

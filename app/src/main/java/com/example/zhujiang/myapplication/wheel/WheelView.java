...
package com.example.zhujiang.myapplication.wheel;
 ...
 *
 * 两个缺点。  1是没有刷新UI的方法， 2是text直接居中的， 不是基于baesline
 */
public class WheelView extends View {

  private final static String TAG = WheelView.class.getSimpleName();

  /** Scrolling duration 滚动持续时间 */
  private static final int SCROLLING_DURATION = 400;

  /** Minimum delta for scrolling 最低三角洲滚动 */
  ...
  private static final int[] SHADOWS_COLORS = new int[] {
          0xCCaabb, 0x000000, 0x224455
  };

  /** Additional items height (is added to standard text item height) 其他项目的高度（被添加到标准文本项目高度 */
  private static final int ADDITIONAL_ITEM_HEIGHT = 15;

  /** Text size */
  private int TEXT_SIZE = 30;//字体大小

  /** Top and bottom items offset (to hide that) 顶部和底部项目抵消（隐藏这一点） */
  ...
  /** Left and right padding value 左和右填充值 */
  private static final int PADDING = 0;

  /** Default count of visible items  可见项目的默认计数 */
  ...
  private void initAttrs(Context context, AttributeSet attrs) {
    ...
    TEXT_SIZE = (int) ta.getDimension(R.styleable.WheelView_wheel_textSize, 14.0f);
    VALUE_TEXT_COLOR = ta.getColor(R.styleable.WheelView_wheel_selectColor, 0xFFFE7900);
    ITEMS_TEXT_COLOR = ta.getColor(R.styleable.WheelView_wheel_normalColor, 0xFFA2A2A2);
  }

  /**
   * Constructor
   */
  //private FontManager fontService;
  public WheelView(Context context) {
    super(context);
    //fontService=new FontManager(context);
    initData(context);
  }
  ...
  private void initResourcesIfNecessary() {
    if (itemsPaint == null) {
      ...
      //itemsPaint.setTypeface(fontService.getTypeface());
    }
  ...
  private int getMaxTextLength() {
    ...
    for (int i = Math.max(currentItem - addItems, 0);
            i < Math.max(currentItem + visibleItems, adapter.getItemsCount()); i++) {
  ...
  private void createLayouts(int widthItems, int widthLabel) {
    ...
    if (widthLabel > 0) {
      if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
        float a = valuePaint.getTextSize();
        //valuePaint.setTextSize(5.0f);
        labelLayout =
                new StaticLayout(label, valuePaint, widthLabel, Layout.Alignment.ALIGN_NORMAL, 1.0f,
                        ADDITIONAL_ITEM_HEIGHT, false);
      } else {
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    if (itemsWidth > 0) {
      ...
      canvas.drawLine(PADDING + VALUE_OFFSET_X + ADDITIONAL_ITEMS_SPACE / 2, y,
              PADDING + VALUE_OFFSET_X + itemsWidth - ADDITIONAL_ITEMS_SPACE / 2, y, valuePaint);
      canvas.drawLine(PADDING + VALUE_OFFSET_X + ADDITIONAL_ITEMS_SPACE / 2, y + itemHeight,
              PADDING + VALUE_OFFSET_X + itemsWidth - ADDITIONAL_ITEMS_SPACE / 2, y + itemHeight,
              valuePaint);
    }
  ...
  private void drawValue(Canvas canvas) {
    ...
    if (labelLayout != null) {
      canvas.save();
      float dx, dy;
      dx = VALUE_OFFSET_X + itemsWidth - ADDITIONAL_ITEMS_SPACE / 2 + LABEL_OFFSET + PADDING;
      dy = bounds.top + LABEL_OFFSET_Y;
      canvas.translate(dx, dy);
      float size = labelLayout.getPaint().getTextSize();
      labelLayout.getPaint().setTextSize(size);
    ...
    if (valueLayout != null) {
      canvas.save();
      canvas.translate(0 + VALUE_OFFSET_X + PADDING, bounds.top + scrollingOffset);
  ...
  private void drawItems(Canvas canvas) {
    ...
    canvas.translate(0 + VALUE_OFFSET_X + PADDING, -top + scrollingOffset);
  ...
  private void doScroll(int delta) {
    ...
    if (pos != currentItem) {
      ...
      Log.w("wheel", "on value test2_ " + currentItem);
      invalidate();
    }
  ...
  public void setTextColor(int select_color, int item_color) {
    this.VALUE_TEXT_COLOR = select_color;
    this.ITEMS_TEXT_COLOR = item_color;
  }
}

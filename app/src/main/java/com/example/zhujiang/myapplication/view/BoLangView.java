...
public class BoLangView extends View implements View.OnClickListener{
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mWaveCount; i++) {
            x1 = -mWaveLength * 3 / 4 + i * mWaveLength + offset;
            x2 = -mWaveLength / 2 + i * mWaveLength + offset;
            Log.w("test", x1 +"," + (mCenterY + 60) + "    " + x2 +"," +mCenterY);
            mPath.quadTo(-mWaveLength * 3 / 4 + i * mWaveLength + offset, mCenterY + 60,
                    -mWaveLength / 2 + i * mWaveLength + offset,mCenterY);

            x1 = -mWaveLength / 4 + i * mWaveLength + offset;
            x2 = i * mWaveLength + offset;
            Log.w("test", x1 +"," + (mCenterY - 60) + "    " + x2 +"," +mCenterY);
    ...
    public void onClick(View v) {
        ...
        mValueAnimatior.setInterpolator(new AccelerateDecelerateInterpolator());
